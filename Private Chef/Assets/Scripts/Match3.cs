...
public class Match3 : MonoBehaviour
{
        ...
        for(int x = 0; x < width; x++)
        {
            for(int y = (height-1); y >= 0; y--)
            {
                ...
                for(int ny = (y-1); ny >= -1; ny--)
                {
                    ...
                    if (nextVal == 0)
                        continue;
                    if(nextVal != -1) //If we did not hit a end,but it's not 0 then use this to fill the current hole
                    {
                    ...
                    {
                        //Use dead ones or create new ones to fill holes (hit -1 ) only if we choose to
    ...
    List<Point> isConnected(Point p, bool main)
    {
        ...
        for (int i = 0; i < 2; i++) //Checking if we are the middle pieceof the same shapes
        {
            List<Point> line = new List<Point>();
